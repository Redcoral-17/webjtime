...
public class TaskService {
    ...
    public TaskService(TaskRepository tRepo, ProjectRepository pRepo, UserService uService) {
        this.tRepo = tRepo;
        this.pRepo = pRepo;
    ...
    public List<Task> getTasksByDate(LocalDate d) {
        return tRepo.findByUserAndDateOrderByStartTimeAsc(uService.getCurrentUser(), d);
    }
    ...
    public void createTask(String n, LocalDate d, LocalTime s, LocalTime e, Project p) {
        ...
        tRepo.save(new Task(n, d, s, e, p, uService.getCurrentUser()));
    ...
    private void checkOwnership(@NonNull Task t) {
        if (!t.getUser().getId().equals(uService.getCurrentUser().getId()))
    ...
    private boolean hasOverlap(LocalDate d, LocalTime s, LocalTime e, Status st) {
        List<Task> tasks = tRepo.findByUserAndDateAndStatus(uService.getCurrentUser(), d, st);
        for (Task task : tasks) {
